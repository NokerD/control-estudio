...
def main():
    ...
    if USUARIO_ACTUAL == "Facundo":
        with st.expander("🍅 Pomodoro 60 min (Proporcional)"):
            st.markdown("Ciclo de 60 min. El tiempo se reparte usando la relación `min_study / objetivo`. Podés pausar y retomar cuando quieras sin que el reloj se rompa.")
            
            # Sacamos el tiempo base que ya tenés acumulado hoy en Google Sheets
            base_est = hms_a_segundos(datos["Facundo"]["tiempos"].get("Estadística I", "00:00:00"))
            base_trab = hms_a_segundos(datos["Facundo"]["tiempos"].get("Trabajo", "00:00:00"))
            
            if st.button("▶ Iniciar Pomodoro", use_container_width=True):
                # Calcular el ratio
                try:
                    min_study = float(st.secrets.get("min_study", 0))
                except (ValueError, TypeError):
                ...
                # Prevenir división por cero si m_obj no está cargado
                m_obj_val = m_obj if m_obj > 0 else 1.0 
                ratio = min_study / m_obj_val
                
                # Limitamos el ratio a 1 por precaución
                ratio = min(ratio, 1.0)
                
                # 60 minutos repartidos proporcionalmente (ej: 0.333 = 20m Est y 40m Trab)
                target_est_min = 60.0 * ratio
                target_trab_min = 60.0 - target_est_min
                
                # Guardamos los estados y las "fotos" del tiempo inicial
                st.session_state["pomo_active"] = True
                st.session_state["pomo_start_est"] = base_est
                st.session_state["pomo_start_trab"] = base_trab
                st.session_state["pomo_target_est"] = target_est_min * 60  # en segundos
                st.session_state["pomo_target_trab"] = target_trab_min * 60 # en segundos
                
                # Arrancar Estadística si no está corriendo
                if st.session_state.get("materia_activa") != "Estadística I":
                    if st.session_state.get("materia_activa") is not None:
                        stop_materia_callback("Facundo", st.session_state["materia_activa"])
                    start_materia_callback("Facundo", "Estadística I")
                
                pedir_rerun()

            # Chequeo del ciclo
            if st.session_state.get("pomo_active", False):
                
                # 1. Calcular cuánto tiempo agregaste DESDE que inició el Pomodoro
                elapsed_est = base_est - st.session_state["pomo_start_est"]
                if st.session_state.get("materia_activa") == "Estadística I":
                    elapsed_est += tiempo_anadido_seg
                    
                elapsed_trab = base_trab - st.session_state["pomo_start_trab"]
                if st.session_state.get("materia_activa") == "Trabajo":
                    elapsed_trab += tiempo_anadido_seg

                target_est_seg = st.session_state["pomo_target_est"]
                target_trab_seg = st.session_state["pomo_target_trab"]
                
                # 2. Lógica de transición Automática
                if elapsed_est >= target_est_seg and elapsed_trab < target_trab_seg:
                    # Se completó Estadística I, pasar a Trabajo
                    if st.session_state.get("materia_activa") == "Estadística I":
                        stop_materia_callback("Facundo", "Estadística I")
                        start_materia_callback("Facundo", "Trabajo")
                        st.success("🔄 ¡Tiempo de Estadística completado! Pasando a Trabajo.")
                        
                elif elapsed_trab >= target_trab_seg:
                    # Se completaron los 60 minutos totales
                    if st.session_state.get("materia_activa") in ["Estadística I", "Trabajo"]:
                        stop_materia_callback("Facundo", st.session_state["materia_activa"])
                    st.session_state["pomo_active"] = False
                    st.success("🍅 ¡Pomodoro de 60 minutos completado con éxito!")
                
                # 3. Interfaz Visual
                if st.session_state.get("pomo_active", False):
                    faltan_est = max(0, target_est_seg - elapsed_est)
                    faltan_trab = max(0, target_trab_seg - elapsed_trab)
                    
                    if faltan_est > 0:
                        st.info(f"🍅 **Estadística I:** Faltan {int(faltan_est // 60)} min {int(faltan_est % 60):02d} seg.")
                    else:
                        st.info(f"🍅 **Trabajo:** Faltan {int(faltan_trab // 60)} min {int(faltan_trab % 60):02d} seg.")
                    
                    # Refresco automático cada 30s SOLO si estás estudiando (si pausás, no gasta batería ni API)
                    if st.session_state.get("materia_activa") is not None:
                        st.markdown('<meta http-equiv="refresh" content="30">', unsafe_allow_html=True)
                
                if st.button("⛔ Cancelar Pomodoro", use_container_width=True):
                    st.session_state["pomo_active"] = False
                    pedir_rerun()
    # ---------------------------------------------------------
